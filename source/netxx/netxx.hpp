...
namespace pollc {
    ...
    enum timeout : int16_t { immediately = 0,
        forever = -1 };

    enum poll_ret : int16_t { poll_timeout = 0,
        poll_error = -1 };
} // namespace pollc
...
namespace settings {
    enum aifamily : int32_t {
        ...
        irda = AF_IRDA, // The Infrared Data Association(IrDA) address family.This
                        // address family is only supported if the computer has an
                        // infrared portand driver installed.
    };
...
public:
    ...
    {
        std::stringstream ss;
        ss << error << NET_OS_MESSAGE << m_errorCode << " return code "
           << status_code;
        m_error = ss.str();
    }
...
void throw_exception_on_poll(const int32_t ret_status,
    const char* call_placement)
{
    if (ret_status > 0)
        return;
...
socket_t make_server(net::settings::server_t& setting, const char* address,
    int32_t port, net::socket::type sock_param)
{
    ...
    net::throw_exception_on(sockServer < 0,
        "Netlib: server create socket failed");

    constexpr int32_t on = 1;
    int32_t ret = ::setsockopt(sockServer, SOL_SOCKET, SO_REUSEADDR, (char*)&on,
        sizeof(on));
    net::throw_exception_on(
        ret < 0, "Netlib: asynchronous server set socket option failed");

    if (sock_param == net::socket::type::nonblocking) {
        ret = ::ioctl(sockServer, FIONBIO, &sock_param);
        net::throw_exception_on(ret < 0,
            "Netlib: asynchronous server ioctl call failed");
    }
    ...
    // UDP is not supported listening?
...
socket_t make_connection(settings::connection_t& setting, const char* address,
    const char* port, net::socket::type sock_param)
{
    socket_t sockConnection = ::socket(setting.aiFamily, setting.aiSocktype, setting.aiProtocol);

    constexpr int32_t on = 1;
    int32_t ret = ::setsockopt(sockConnection, SOL_SOCKET, SO_REUSEADDR,
        (char*)&on, sizeof(on));
    net::throw_exception_on(ret < 0,
        "Netlib: connection set socket option failed");
    ...
    if (sock_param == net::socket::type::nonblocking) {
        ...
        do {
            ...
            std::cout << "Connection async status: " << ret << " errno: " << errno
                      << &std::endl;
        } while (ret != 0);
        std::cout << "Connection async status ready: " << ret << " errno: " << errno
                  << &std::endl;
    } else {
        ret = ::connect(sockConnection, (struct sockaddr*)&addr, sizeof(addr));
...
int32_t wait_connection(net::socket_t& sock_server, socket_t& sock_client,
    int32_t connections)
{
    net::throw_exception_on(sock_client > 0,
        "Netlib: provided client is not closed");

    do {
        sock_client = ::accept(sock_server, nullptr, nullptr);
    } while (sock_client < 0 && (errno == EAGAIN || errno == EWOULDBLOCK));
    if (sock_client < 0) {
        std::cout << "Asynchronous server client socket got accept error"
                  << &std::endl;
        return 0;
    }

    std::cout << "Server got connection socket id ... " << sock_client
              << &std::endl;
    return 1;
}
...
void set_pollfd(net::pollfd_s pollfd_array[], uint64_t array_len,
    net::pollc::param param)
{
...
void interpret_address(socket_t& sockfd, ip_address_s& ip_addr)
{
    ...
    int32_t ret = ::getpeername(sockfd, (struct sockaddr*)(&sockAddress),
        (socklen_t*)(&szAddress));
    ...
    ip_addr.type = (ip_addr.addr_size == INET6_ADDRSTRLEN)
        ? net::settings::aifamily::inetv6
